...
%      Thus $\F f$ is a Schwartz function, and so, because $f(-x)$ is also Schwartz, $\F^{-1}f$ is \emph{also} Schwartz.
    ...
    \section{Tempered Distributions}
      ...
      \end{align*}
      We thus \emph{define} the Fourier transform of a general $f\in\S^*$ by the adjoint-like characterization:
      \begin{equation*}
        \langle\F f,\varphi\rangle = \langle f,\F\varphi\rangle \text{ for }f,\varphi\in\S^*
      \end{equation*}
      The key point is that this definition is \emph{consistent} with our viewing Schwartz functions as tempered distributions---in fact, it is the only such way to define \F on $\S^*$.

      Having defined the Fourier transform on tempered distributions, we hope to show that it behaves nicely.
      Happily, it inherits much of its behavior from its action on Schwartz functions.
      For example:
      \begin{thm}
        Let $\varphi\in\S^*$.
        Then:
        \begin{align}
          \label{eq:tsltispsi_td}
            T_x\F^{-1}\varphi&=\F^{-1}(\psi_x\varphi)\\
          \label{eq:xisdiff_td}
            \F(x\varphi) &= -\frac{1}{2\pi i}(\F \varphi)'\\
          \label{eq:fourinv_td}
          \F^{-1}\F\varphi &= \varphi
        \end{align}
      \end{thm}
      \begin{proof}[Proof of \eqref{eq:tsltispsi_td}]
      \end{proof}
      \begin{proof}[Proof of \eqref{eq:xisdiff_td}]
      \end{proof}
      \begin{proof}[Proof of \eqref{eq:fourinv_td}]
      \end{proof}
