\input{chpreamble}
  \chapter{Fourier Inversion}
    ...
    We hope that $\F^{-1}$ does in fact invert $\F$, so that for any function $f$, $\F^{-1}\F f = f$---that is, applying the inverse Fourier transform to the frequency spectrum of $f$ gives us back $f$.
      ...
      \begin{example}[The heat equation]
        ...
        \begin{figure}[p]
          \begin{center}
            ...
            \end{sagesilent}
            \sageplot{reduce(lambda x,y: x+y,[plot(u(x,t),x,(-1,2),color=c,legend_label='$t='+str(t)+'$',axes=false,figsize=[6,4]) for t,c in [(1/1000,(0,0,0)), (1/32,(.1,.1,.1)), (1/8,(.3,.3,.3)), (1/4,(.4,.4,.4)), (1/2,(.5,.5,.5)), (1,(.6,.6,.6)), (2,(.7,.7,.7))]])}
          \end{center}
...
%      This is demonstrated by the ease of proving Fourier inversion:
      \begin{thm}[Fourier Inversion on $\S^*$]
        ...
        \end{align*}
      \end{thm}
      \begin{proof}[Proof of \cref{thm:fourinv_td}]
        For $\phi\in\S$:
        \begin{align*}
