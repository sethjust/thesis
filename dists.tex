...
%    so for any $\delta,\varepsilon>0$ there is some $N$ so that $n>N$ implies $\int_{|x|\ge\delta}\phi_n(x)\,dx<\varepsilon$.
    ...
    In fact, we may define convolution by a simpler, vector-valued integral.
    Compactly supported \todo{define this term} maps $a:\R\rightarrow\S$ have a Gelfand--Pettis integral \todo{why?}
    \begin{align*}
      A = \int_\R a
    \end{align*}
    ...
    \todo{this is total bunk; we can't say that that integral exists}
...
%      \\&= \left\langle u, \int_{x\in\R} \tilde\phi(y-x)\varphi(x)\,dx \right\rangle
    ...
    \end{align*}
    with the second equality uses the properties of the Gelfand--Pettis integral on \S. \todo{once again this is bunk because the integral may not exist without compact support.}
