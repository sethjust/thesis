...
%        \le \int_{|x|<\delta} \phi_n(x)|\varphi(x)-\varphi(0)|\,dx
  ...
  \subsection{Smoothing Distributions}
    For some measure space $X$, compactly supported maps $a:X\rightarrow\S$ have a Gelfand--Pettis integral \todo{why?} \todo{do compact sets always have finite measure?}
    \begin{align*}
      A = \int_\R a(x)\,dx \in\S
    \end{align*}
    ...
    \end{align*}
    However, proving the integral exists is a difficult claim, as the existence of the Gelfand--Pettis integral depends on integrating over a compact set with finite measure, which is only the case for $\int_\R a$ because $f$ is compactly supported, while $g$ is not.
    ...
    \begin{proof}
      ...
      Thus
      \begin{align*}
        \langle \delta_x, \int_{\R} g(y)T_{-y}f\,dy \rangle
    ...
    \end{lemma}
    \begin{proof}
      \todo{change this proof from pointwise to topological convergence.}
