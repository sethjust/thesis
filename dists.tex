\input{chpreamble}
  ...
  \begin{proof}
    ...
    For any $f\in\S$ let $u_f$ be the distribution given by integration against $f$.
    Then
    \begin{align*}
      |\langle u_f,\phi_i\rangle| 
      &= \left|\int_{x\in\R} f(x)\phi_i(x)\,dx\right|
      \le |\phi_i(x)|_\Coo \cdot \int_{x\in\R} |f(x)|\,dx
%      \\&\le |\phi_i|_{C^k_\ell}\cdot\int_{x\in\R}|\varphi(x)|\,dx
    \end{align*}
    Because $\{\phi_i\}$ approaches zero in \S, $\{|\phi_i|_\Coo\}$ also approaches zero.
    Thus the sequence $\{\langle f,\phi_i\rangle\}$ approaches zero in \C, so the functional is continuous.
  \end{proof}
  ...
  In fact, for a sequence $\{\phi_i\}\rightarrow 0$ in \S and any $\ell\in\Z_{>0}$, see that $\{x^\ell\phi_i(x)\}\rightarrow 0$ for any fixed $x\in\R$, and so by \cref{thm:sevalcts} $\{x^\ell\phi_i\}\rightarrow 0$.
  Thus, pretending that some function $f$ does give rise to a distribution $u_f$, write for any $\ell\in\Z_{>0}$:
  \begin{align*}
    |\langle u_f,\phi_i\rangle| 
    \le |x^\ell\phi_i(x)|_\Coo \cdot \int_{x\in\R} |x^{-\ell}f(x)|\,dx
  \end{align*}
  which approaches zero as $i$ grows, provided the integral is finite.
  Thus a classical function $f$ gives rise to a tempered distribution $u_f$ whenever there is an $\ell$ so that $x^{-\ell}f$ is $L^1$-integrable.

  \begin{defn}
    A continuous function $f$ is called a \emph{slow-growth function} if there exists some $\ell\in\Z_{\ge0}$ so that
    \begin{align*}
      \int_{x\in\R} |x^{-\ell} f(x)|\,dx < \infty \text{.}
    \end{align*}
  \end{defn}
  The discussion above immediately shows that any slow-growth function $f$ gives rise to a tempered distribution $u_f$ by integration-against-$f$.
  Particularly, any polynomial is a slow-growth function, and so gives rise to a tempered distribution \todo{this is false with defn. given}.
  However, the exponential function $e^x$ is not of slow-growth, as it grows faster than any power of $x$ for large $x$.
%  We wonder if the converse of this is also true, so the only functions giving rise to distributions are slow-growth functions.
%
%  \begin{thm}
%    If a distribution $u$ is given by integration against a classical function $f$, then $f$ is of slow growth.
%  \end{thm}
%  \begin{proof}
...
%        \\&= \left\langle u, x\mapsto \int_{y\in\R} \phi_n(y)T_xf(y)\,dy \right\rangle
    ...
    Unfortunately, arbitrary mollifications need not be Schwartz functions; they might be of slow growth and never vanish as $x$ grows.
