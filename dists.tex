...
%      &= \left|\int_{x\in\R} \varphi(x)\phi_i(x)\,dx\right|
  ...
  \section{Approximation by Schwartz Functions}
    ...
    \end{align*}
    Because $f$ and $g$ are Schwartz, see that $f*g$ is a smooth function of $x$ becaus the $x$ derivative passes through the $y$ integral and attaches itself to the $f$ term.
    For very large $x$, we have that for any $y\in\R$ at least one of $|x-y|$ or $|y|$ is large, and so either $f(x-y)$ or $g(y)$ is small.
    Thus as $x$ grows we see that $(f*g)(x)$ will eventually become small, so $f*g$ is Schwartz.
