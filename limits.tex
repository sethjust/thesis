\input{chpreamble}
  \chapter{Products and Limits of Topological Spaces}
    ...
    \todo{say anything about closedness}
    ...
    We define the product of a collection of topological spaces by a mapping property; this definition will \emph{force} the product to assume a unique topology.
      ...
      \begin{claim}
        ...
        \end{rmk}
        \begin{proof}[Proof of \thref{claim:prodiscart}]
...
%        \end{align*}
    ...
    \section{Projective Limits}
      ...
      To get a more intuitive idea of the limit topology, let $X$ be the limit of topological spaces $X_i$ with an open subset $U$.
      ...
      \todo{use something out of this mess to finish this discussion}

%      Denote the inclusion $X\rightarrow Y$ by $\tilde{\imath}$.
%      Then $\tilde{\imath}(U) \subseteq \bigcap_{i=1}^k p_{j_i}^{-1}(u_{j_i})$ for finitely many $u_{j_i}$ open in $X_{j_i}$.
      
%      Recall that elements of $Y$ are collections of one element from each $X_i$, and that $p_i(\{x_i:i\in A\})=x_i$.

%      The inverse image of an open $u_i\subseteq X_i$ is the set
%      \begin{align*}
%        \left\{ x\in Y : p_i(x)\in u_i \right\}\text{.}
%      \end{align*}
%%      or equivalently
%%      \begin{align*}
%%        \left\{ \{x_j:j\in A\}\in X : x_i\in u_i \right\}\text{.}
%%      \end{align*}
%      For $u_i,u_i'\subseteq X_i$, we see that
%      \begin{align*}
%        p_i^{-1}(u_i)\cup p_i^{-1}(u_i')
%        &= p_i^{-1}(u_i\cup u_i')
%      \end{align*}
%      and
%      \begin{align*}
%        p_i^{-1}(u_i)\cap p_i^{-1}(u_i')
%        &= p_i^{-1}(u_i\cap u_i')
%      \end{align*}
%      which shows the product topology inheirits its properties from the topology of the factors.
%      Now consider open $u_i\subseteq X_i$ and $u_j\subseteq X_j$ for $i\ne j\in A$.
%      Then
%      \begin{align*}
%        p_i^{-1}(u_i)\cup p_j^{-1}(u_j)
%        &= \left\{ x\in Y:p_i(x)\in u_i\text{ or }p_j(x)\in u_j \right\}
%      \end{align*}
%      and
%      \begin{align*}
%        p_i^{-1}(u_i)\cap p_j^{-1}(u_j)
%        &= \left\{ x\in Y:p_i(x)\in u_i\text{ and } p_j(x)\in u_j \right\}
%      \end{align*}
    ...
    \section{Indexing by Larger Sets}
      ...
      \end{displaymath}
      Note that this definition of a limit matches \thref{defn:projlim} except for allowing more general indexing.
      Notably, proofs of uniqueness and construction as a subset of the product exist for projective systems indexed by a poset.
      \todo{double check that}
      ...
      \end{itemize}
      \begin{thm}[Cofinal Limits are Isomorphic]
