\input{chpreamble}
  ...
  %  \footnote{That many seemingly sensible problems lead to such results, at least in the context of elementary mathematics, is not only a frustration for students, but a failing of mathematics.}
    ...
    However, given that a thermometer is not sensitive to temperatures far away from itself, $\varphi$ should take value $0$ for $x$ away from $[0,\ell]$, and so we can write $T_\text{measured} = \int_{-\infty}^{\infty} f(x)\varphi(x)dx$ without changing the result.
