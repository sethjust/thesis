...
%          Thus when $f$ and $g$ are close in $C^k_0$, $Df$ and $Dg$ are close in $C^{k-1}_0$.
      ...
      \end{thm}
      \begin{proof}
        \todo{IMPORTANT!!! Fix this proof so that it holds well; namely, ensure that the relationship between the $C^k_\ell$ and $C^k$ norms is treated properly.}
...
%        To show that $\S=\lim_{k,\ell}C^k_\ell$ we need only show that $\S$ meets the definition of limit for some cofinal directed subfamily of the $C^k_\ell$.
      ...
      \todo{this description of open sets seems to imply that \S is dense in $C^k_\ell$, but I need to examine it.}
    ...
    \section{Consequences of Topology on \S}
      ...
      \begin{claim}
        ...
        \todo{ismorphism?}
        \begin{proof}
          Consider some open $U\subseteq\S$ and some $\varphi\in\S$ such that $D\varphi\in U$.
          Because $U$ is open, there is some $C^k_\ell$ and open $u\subseteq C^k_\ell$ such that $u\cap\S\subseteq U$, and $D\varphi\in u$.
          Pick $\varepsilon$ such that the open ball in $C^k_\ell$ with radius $\varepsilon$ and centered at $D\varphi$ is contained by $u$.

%          Now pick $\phi\in\S$ such that $|\varphi-\phi|_{C^k_\ell}<\varepsilon$.
By the continuity of differentiation $D:C^k_\ell\rightarrow C^{k-1}_\ell$ from \thref{claim:diffcontCkl}, there is some $\delta>0$ such that for $\phi\in C^{k+1}_\ell$ with $|\varphi-\phi|_{C^{k+1}_\ell}<\delta$, we have $|D\varphi-D\phi|_{C^k_\ell}<\varepsilon$.
          Intersecting the radius-$\delta$ ball with \S yields an open subset of \S containing $\varphi$, whose image under $D$ is contained in $u\cap\S$.
          Thus, because each point in $D^{-1}(U)$ is surrounded by an open ball whose image under differentiation is in $U$, see that $D:\S\rightarrow\S$ is continuous.
        \end{proof}
      \end{claim}
