...
%        To show that $\S=\lim_{k,\ell}C^k_\ell$ we need only show that $\S$ meets the definition of limit for some cofinal directed subfamily of the $C^k_\ell$.
      ...
      \begin{defn}
        ...
        See \citet{} for a full treatment of different definitions of Fr\'echet spaces. \todo{cite Paul}
      \end{defn}
      ...
      But because $\varphi_i$ is inclusion, its inverse image is exactly intersection.
      Thus a subset $U$ of \S is open if and only if for each point $u\in U$ there is some $j\in A$ so that $u\in V$ where $V\subseteq X_j$ is open, and $V\cap\S\subseteq U$.
