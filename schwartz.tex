...
%    By working in the category of continuous maps between topological spaces we are able to avoid much of the clutter of a classical working environment.
    ...
    \section{Spaces of Functions}
      ...
      \begin{defn}
        ...
        Given a set $S$ of subsets of $X$ that contains $\emptyset$ and $X$, generate a topology $\tau$ on $X$ by completing $S$ with respect to union and finite intersection (that is, taking all possible unions and intersections of sets in $S$ repeatedly until no new open sets are generated).
        $S$ is called a \emph{generating set} or \emph{basis} of the topology $\tau$.
